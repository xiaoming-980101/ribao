/**
 * 赢日志 - 智能日志生成与查重引擎 (口语化去AI感、支持前端/设计多岗位版)
 */

// 1. 各岗位日常/无任务模板语料库 (口语化、流水账，彻底消灭AI感)
  ...
  frontend: {
    actions: [
      [
        ...
        '在一些核心的代码里加了点空值校验 and 容错处理，防患未然',
...
const JOB_TITLES: Record<string, { random: string[]; study: string[] }> = {
  frontend: {
    random: ['前端公共逻辑提取与优化', '日常代码整理与无用依赖清理', '项目打包配置优化与调试', '页面卡顿优化与缺陷修复', '冗余代码清理与组件整理'],
    study: ['React并发机制学习', '打包优化方案调研', 'TS高级类型学习', '微前端方案预研', '前端单元测试学习']
  },
  designer: {
    random: ['UI设计规范整理与统一', '前端页面视觉走查反馈', '设计素材收集与资源归档', '设计稿图层规范化整理', '切图标注交付与动效对接'],
...
export function expandUserInput(userInput: string, job: string = 'frontend'): GeneratedLogResult {
  ...
  tasks.forEach((task, index) => {
    ...
    if (matchedRule) {
      ...
      lines.push(`${index + 1}. 完成了[${task}]相关工作，${baseExpand}`);
    }
  });
...
3. 增加一点点口语化的工作细节（比如“把 Figma 的间距标注仔细对了一遍”、“把控制台里的几个警告日志清理了一下”等），让日报显得极其逼真。
