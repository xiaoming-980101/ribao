...
export function generateAIPrompt(userInput: string, job: string): string {
  ...
  const tasksText = userInput.trim()
    ? `【${userInput.trim()}】`
    : '“日常基础代码库维护与细节调优：排查小问题、整理代码结构、清理警告日志并做本地自测”';
...
`;
  ...
  return `你是一个专业 ${jobName}，擅长把当天真实工作记录整理成平实、简洁的公司内部日报。请根据下面的工作记录，帮我写一份日常工作日志。
...
3. 字数控制在 100 - 150 字之间，分 2-3 条列出。
4. 每条内容保持 20 到 35 个字左右，句子要完整，有动作、步骤或自测细节。
